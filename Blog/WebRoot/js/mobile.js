(function(root, factory) {
	...
	Mobile.prototype.renderDOM = function(){
		...
		var navStr = '<span class="viewer-title">导航</span><div class="viewer-div"><a href="../FaceBlog/index.jsp">主页</a></div>';
	...
	Mobile.prototype.bindDOM = function(){
		...
		//滑动隐藏
		var vb=document.getElementById("viewer-box");
		if(vb.attachEvent){
		vb.attachEvent("onwebkitTransitionEnd", function(){
			...
			//点击展示和隐藏
		self.ctn.attachEvent("ontouchend", function(){
			self.show();
		}, false);
		}
		if(vb.addEventListener){
			vb.addEventListener("webkitTransitionEnd", function(){
		...
		var $right = $(".viewer-box-r")[0] || document.getElementsByClassName("viewer-box-r")[0];;
		var touchStartTime;
		var touchEndTime;
		if($right.attachEvent){
			$right.attachEvent("ontouchstart", function(){
			touchStartTime = + new Date();
			}, false);
			$right.attachEvent("ontouchend", function(){
		...
		else if($right.addEventListener){
